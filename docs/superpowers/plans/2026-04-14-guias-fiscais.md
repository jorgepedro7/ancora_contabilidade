# ObrigacaoGuiaView — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Substituir o placeholder de `ObrigacaoGuiaView.vue` por um calendário mensal de obrigações fiscais com grupos colapsáveis por mês e modal de registro de pagamento.

**Architecture:** Implementação puramente frontend — um único arquivo Vue 3 (`<script setup>`) que consome a API existente via `obrigacoesService`. O agrupamento por mês, filtro por tipo e cálculo de totais são feitos no frontend com `computed`. Nenhuma mudança de backend, model ou migration.

**Tech Stack:** Vue 3 Composition API, Pinia (`useUiStore`), Axios via `obrigacoesService`, Tailwind CSS com tokens Ancora.
...
|---|---|---|
| `frontend/src/views/obrigacoes/ObrigacaoGuiaView.vue` | Substituir (rewrite completo) | Calendário mensal + modal de pagamento |

Nenhum outro arquivo é alterado.

---

## Task 1: Script setup — dados, computeds e funções auxiliares

**Files:**
- Modify: `frontend/src/views/obrigacoes/ObrigacaoGuiaView.vue`

**Contexto para o implementador:**

O arquivo atual é um placeholder de 8 linhas. Substitua-o inteiramente. O service disponível está em `frontend/src/services/obrigacoes.service.js` e exporta `obrigacoesService` como default com os métodos:
- `getObrigacoes(params = {})` → retorna `{ results: [...], count, ... }`
- `updateObrigacao(id, data)` → PATCH

A store de UI está em `frontend/src/stores/ui` e expõe `uiStore.showNotification(msg, 'success'|'error')`.

- [ ] **Step 1: Escrever o `<script setup>` completo**

Escreva o arquivo com apenas a seção `<script setup>` por enquanto (o template vem na Task 2). O arquivo deve conter:

```vue
<template>
  <div class="p-4"><!-- template vem na Task 2 --></div>
</template>
...
node -e "
const fs = require('fs');
const content = fs.readFileSync('frontend/src/views/obrigacoes/ObrigacaoGuiaView.vue', 'utf8');
if (content.includes('<script setup>') && content.includes('fetchObrigacoes') && content.includes('mesesVisiveis') && content.includes('contadorMes')) {
  console.log('OK — script setup presente com todas as funções esperadas');
} else {
  console.error('ERRO — funções ausentes');
  process.exit(1);
}
"
```

Esperado: `OK — script setup presente com todas as funções esperadas`

---

## Task 2: Template completo — grupos colapsáveis + modal
...
Substitua o `<template>` placeholder pelo template completo abaixo. Mantenha o `<script setup>` da Task 1 intacto.

- [ ] **Step 1: Substituir o `<template>` pelo template completo**
...
- [ ] **Step 2: Verificar que o arquivo final tem `<template>`, `<script setup>` e nenhum placeholder**
...
const checks = [
  ...
  ['<script setup>', content.includes('<script setup>')],
  ['mesesVisiveis', content.includes('mesesVisiveis')],
  ['confirmarPagamento', content.includes('confirmarPagamento')],
  ['toggleMes', content.includes('toggleMes')],
  ['sem placeholder', !content.includes('em desenvolvimento')],
];
let ok = true;
...
```bash
git add frontend/src/views/obrigacoes/ObrigacaoGuiaView.vue
git commit -m "feat(obrigacoes): implementa calendário mensal de guias fiscais

- Grupos colapsáveis por mês (3 passados + atual + 3 futuros)
- Filtro por tipo de obrigação
- Badge de status com cores (ABERTO/ATRASADO/PAGO)
- Modal de registro de pagamento com data, comprovante e observações
...
## Task 3: Verificar rotas e atualizar PROGRESSO.md

**Files:**
- Read: `frontend/src/router/index.js`
- Modify: `docs/PROGRESSO.md`

- [ ] **Step 1: Confirmar que a rota `/obrigacoes/guias` já aponta para o componente correto**

```bash
grep -n "ObrigacaoGuiaView\|obrigacoes/guias" frontend/src/router/index.js
```

Esperado: linha com `path: '/obrigacoes/guias'` e `ObrigacaoGuiaView`.

- [ ] **Step 2: Atualizar PROGRESSO.md — Módulo 9 de ⚠️ para ✅**

No arquivo `docs/PROGRESSO.md`, localizar a seção do Módulo 9 e alterar:
- Status de `⚠️ Parcial` para `✅ Completo`
- Remover a linha `**Gap pendente:** ObrigacaoGuiaView.vue é placeholder...`
- Adicionar em Fixes aplicados: `- ObrigacaoGuiaView.vue: implementado calendário mensal com grupos colapsáveis e modal de pagamento`
- Remover o item 1 de "Próximos passos sugeridos" (sobre ObrigacaoGuiaView)

- [ ] **Step 3: Commit final**

```bash
git add docs/PROGRESSO.md
git commit -m "docs: Módulo 9 completo — ObrigacaoGuiaView implementada"
```
