<template>
  <div class="p-4">
    <div class="mb-6 flex items-center justify-between">
      <div>
        <h1 class="text-3xl font-display text-ancora-gold mb-1">Contas a Pagar</h1>
        <p class="text-sm text-gray-400">Compromissos financeiros da empresa ativa.</p>
      </div>
    ...
    <div class="mb-4 flex flex-wrap gap-3">
      ...
      <select v-model="filterStatus" @change="fetchContas"
        ...
        <option value="PAGA_RECEBIDA">Paga</option>
      ...
      <table class="min-w-full bg-ancora-black/50 text-white">
        <thead>
          <tr class="bg-ancora-navy text-ancora-gold uppercase text-sm leading-normal">
            ...
            <th class="py-3 px-4 text-right">Pago</th>
        ...
        <tbody class="text-sm font-body">
          <tr v-for="conta in contas" :key="conta.id"
            ...
            <td class="py-3 px-4">{{ conta.fornecedor_detail?.nome_razao_social }}</td>
            ...
            <td class="py-3 px-4 text-right font-mono">{{ formatCurrency(conta.valor_pago) }}</td>
            ...
            <td class="py-3 px-4 text-center whitespace-nowrap">
              ...
              <button v-if="conta.status !== 'PAGA_RECEBIDA' && conta.status !== 'CANCELADA'"
                      @click="openPagarModal(conta)"
                      class="text-green-400 hover:text-green-300 font-bold mr-2">Pagar</button>
    ...
    <div v-else-if="!loading" class="text-center text-gray-400 mt-8">Nenhuma conta a pagar encontrada.</div>
    ...
    <div v-if="isModalOpen" class="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-ancora-black/70 p-4">
      <div class="max-h-[calc(100vh-2rem)] w-full max-w-xl overflow-y-auto rounded-lg border border-ancora-gold/30 bg-ancora-black/90 p-6 shadow-xl">
        <h2 class="text-2xl font-display text-ancora-gold mb-5">{{ editingConta ? 'Editar Conta a Pagar' : 'Nova Conta a Pagar' }}</h2>
        <form @submit.prevent="saveConta" class="space-y-4">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label class="block text-sm text-gray-300 mb-1">Fornecedor *</label>
              <select v-model="form.fornecedor" required
                      class="block w-full px-3 py-2 bg-ancora-black/70 border border-ancora-gold/20 rounded-md text-white sm:text-sm">
                <option :value="null" disabled>Selecione...</option>
                <option v-for="f in fornecedoresList" :key="f.id" :value="f.id">{{ f.nome_razao_social }}</option>
              </select>
            </div>
            <div>
              <label class="block text-sm text-gray-300 mb-1">Conta Contábil (Despesa) *</label>
    ...
    <!-- Modal Pagar -->
    <div v-if="pagarModal.open" class="fixed inset-0 z-50 flex items-center justify-center bg-ancora-black/70 p-4">
      <div class="w-full max-w-md rounded-lg border border-green-500/30 bg-ancora-black/95 p-6 shadow-xl">
        <h3 class="text-lg font-display text-green-400 mb-1">Registrar Pagamento</h3>
        <p class="text-sm text-gray-400 mb-4">{{ pagarModal.descricao }}</p>
        <div class="space-y-4">
          <div>
            <label class="block text-sm text-gray-300 mb-1">Valor a Pagar (R$) *</label>
            <input type="number" step="0.01" min="0.01" :max="pagarModal.saldo" v-model="pagarModal.valor" required
                   class="block w-full px-3 py-2 bg-ancora-black/70 border border-ancora-gold/20 rounded-md text-white sm:text-sm"/>
            <p class="text-xs text-gray-500 mt-1">Saldo: {{ formatCurrency(pagarModal.saldo) }}</p>
          </div>
          <div>
            <label class="block text-sm text-gray-300 mb-1">Conta Bancária *</label>
            <select v-model="pagarModal.contaBancariaId" required
                    class="block w-full px-3 py-2 bg-ancora-black/70 border border-ancora-gold/20 rounded-md text-white sm:text-sm">
              <option :value="null" disabled>Selecione...</option>
        ...
        <div class="flex justify-end gap-3 mt-5">
          <button @click="pagarModal.open = false" class="px-4 py-2 bg-gray-600 text-white rounded-md hover:bg-gray-700">Cancelar</button>
          <button @click="confirmarPagar" :disabled="saving || !pagarModal.contaBancariaId"
...
const pagarModal = reactive({ open: false, id: null, descricao: '', saldo: 0, valor: 0, contaBancariaId: null })
...
async function fetchContas() {
  ...
  try {
    ...
    const response = await FinanceiroService.getContasAPagar(params)
    contas.value = response.results
  } catch {
    uiStore.showNotification('Falha ao carregar contas a pagar.', 'error')
  } finally {
    loading.value = false
  }
}
...
async function loadFornecedores() {
  try {
    const r = await CadastrosService.getFornecedores({ page_size: 500 })
    fornecedoresList.value = r.results
  } catch { /* silencioso */ }
}

async function loadPlanoContas() {
  try {
    const r = await FinanceiroService.getPlanosContas({ page_size: 500, tipo_conta: 'DS' })
...
async function saveConta() {
  ...
  try {
    ...
    if (editingConta.value) {
      await FinanceiroService.updateContaAPagar(form.id, payload)
...
function openPagarModal(conta) {
  pagarModal.open = true
  pagarModal.id = conta.id
  pagarModal.descricao = conta.descricao
  pagarModal.saldo = Number(conta.valor_saldo)
  pagarModal.valor = Number(conta.valor_saldo)
  pagarModal.contaBancariaId = contasBancariasList.value[0]?.id || null
}

async function confirmarPagar() {
  saving.value = true
  try {
    await FinanceiroService.pagarContaAPagar(pagarModal.id, {
      valor: pagarModal.valor,
      conta_bancaria_id: pagarModal.contaBancariaId,
    })
    uiStore.showNotification('Pagamento registrado!', 'success')
    pagarModal.open = false
    await fetchContas()
    await loadContasBancarias()
  } catch (err) {
    uiStore.showNotification(extractApiErrorMessage(err, 'Erro ao registrar pagamento.'), 'error', 6000)
  } finally {
    saving.value = false
  }
}
...
async function confirmarDelete() {
  saving.value = true
  try {
    await FinanceiroService.deleteContaAPagar(deleteModal.id)
...
function statusLabel(s) {
  return { ABERTA: 'Aberta', PARCIAL: 'Parcial', PAGA_RECEBIDA: 'Paga', CANCELADA: 'Cancelada' }[s] || s
}
